...
function setCurrentTab() {}	
	...
	boxxjs.pub = 0x28; // 40
	boxxjs.priv = 0xd2; // 210
	boxxjs.multisig = 0x0a; // 10
	boxxjs.wif = 0xd2; // 210
	boxxjs.hdkey = {'prv':0x0488ade4, 'pub':0x0488b21e};
	boxxjs.compressed = false;
	...
	boxxjs.newPubkey = function(hash){
	...
	boxxjs.pubkey2address = function(h, byte){
		var r = ripemd160(Crypto.SHA256(Crypto.util.hexToBytes(h), {asBytes: true}));
		r.unshift(byte || boxxjs.pub);
		var hash = Crypto.SHA256(Crypto.SHA256(r, {asBytes: true}), {asBytes: true});
		var checksum = hash.slice(0, 4);
		return boxxjs.base58encode(r.concat(checksum));
	}

	/* provide a scripthash and return address */
	boxxjs.scripthash2address = function(h){
		var x = Crypto.util.hexToBytes(h);
		x.unshift(boxxjs.pub);
		var r = x;
		r = Crypto.SHA256(Crypto.SHA256(r,{asBytes: true}),{asBytes: true});
		var checksum = r.slice(0,4);
		return boxxjs.base58encode(x.concat(checksum));
	}

	/* new multisig address, provide the pubkeys AND required signatures to release the funds */
	boxxjs.pubkeys2MultisigAddress = function(pubkeys, required) {
		var s = boxxjs.script();
		...
		x.unshift(boxxjs.multisig);
		var r = x;
		r = Crypto.SHA256(Crypto.SHA256(r, {asBytes: true}), {asBytes: true});
		var checksum = r.slice(0,4);
		var redeemScript = Crypto.util.bytesToHex(s.buffer);
		var address = boxxjs.base58encode(x.concat(checksum));
	...
	*/
	boxxjs.simpleHodlAddress = function(pubkey, checklocktimeverify) {
		...
		var s = boxxjs.script();
		s.writeBytes(boxxjs.numToByteArray(checklocktimeverify));
	...
	boxxjs.segwitAddress = function(pubkey){
	...
	boxxjs.bech32Address = function(pubkey){
		var program = ripemd160(Crypto.SHA256(Crypto.util.hexToBytes(pubkey), {asBytes: true}), {asBytes: true});
		var address = boxxjs.bech32_encode(boxxjs.bech32.hrp, [boxxjs.bech32.version].concat(boxxjs.bech32_convert(program, 8, 5, true))); 
	...
	boxxjs.bech32redeemscript = function(address){
		var r = false;
		var decode = boxxjs.bech32_decode(address);
		if(decode){
			decode.data.shift();
			return Crypto.util.bytesToHex(boxxjs.bech32_convert(decode.data, 5, 8, true));
		}
		return r;
	}

	/* provide a privkey and return an WIF  */
	boxxjs.privkey2wif = function(h){
	...
	boxxjs.wif2privkey = function(wif){
		var compressed = false;
		var decode = boxxjs.base58decode(wif);
	...
	boxxjs.wif2pubkey = function(wif){
		var compressed = boxxjs.compressed;
		var r = boxxjs.wif2privkey(wif);
		boxxjs.compressed = r['compressed'];
		var pubkey = boxxjs.newPubkey(r['privkey']);
		boxxjs.compressed = compressed;
		return {'pubkey':pubkey,'compressed':r['compressed']};
	}

	/* convert a wif to a address */
	boxxjs.wif2address = function(wif){
		var r = boxxjs.wif2pubkey(wif);
		return {'address':boxxjs.pubkey2address(r['pubkey']), 'compressed':r['compressed']};
	}

	/* decode or validate an address and return the hash */
	boxxjs.addressDecode = function(addr){
		try {
			var bytes = boxxjs.base58decode(addr);
			...
			if (checksum+"" == back+"") {
				...
				if(o.version==boxxjs.pub){ // standard address
					o.type = 'standard';

				} else if (o.version==boxxjs.multisig) { // multisig address
					o.type = 'multisig';

				} else if (o.version==boxxjs.priv){ // wifkey
					o.type = 'wifkey';
			...
			bech32rs = boxxjs.bech32redeemscript(addr);
			if(bech32rs){
	...
	boxxjs.pubkeydecompress = function(pubkey) {
	...
	boxxjs.bech32_polymod = function(values) {
		var chk = 1;
	...
	boxxjs.bech32_hrpExpand = function(hrp) {
		var ret = [];
		var p;
	...
	boxxjs.	bech32_verifyChecksum = function(hrp, data) {
		return boxxjs.bech32_polymod(boxxjs.bech32_hrpExpand(hrp).concat(data)) === 1;
	}

	boxxjs.bech32_createChecksum = function(hrp, data) {
		var values = boxxjs.bech32_hrpExpand(hrp).concat(data).concat([0, 0, 0, 0, 0, 0]);
		var mod = boxxjs.bech32_polymod(values) ^ 1;
		var ret = [];
		for (var p = 0; p < 6; ++p) {
	...
	boxxjs.bech32_encode = function(hrp, data) {
		var combined = data.concat(boxxjs.bech32_createChecksum(hrp, data));
		var ret = hrp + '1';
		for (var p = 0; p < combined.length; ++p) {
			ret += boxxjs.bech32.charset.charAt(combined[p]);
		}
		return ret;
	}

	boxxjs.bech32_decode = function(bechString) {
		var p;
		var has_lower = false;
		var has_upper = false;
		...
		for (p = pos + 1; p < bechString.length; ++p) {
			var d = boxxjs.bech32.charset.indexOf(bechString.charAt(p));
			if (d === -1) {
				return null;
			}
			data.push(d);
		}
		if (!boxxjs.bech32_verifyChecksum(hrp, data)) {
			return null;
		}
		return {
			hrp: hrp,
	...
	boxxjs.bech32_convert = function(data, inBits, outBits, pad) {
		var value = 0;
		var bits = 0;
	...
	boxxjs.testdeterministicK = function() {
		...
		for (i = 0; i < test_vectors.length; i++) {
			...
			var wif = boxxjs.privkey2wif(test_vectors[i]['privkey']);
	...
	boxxjs.hd = function(data){
		...
		r.parse = function() {
			...
			if(typeof(data) == 'string'){
				var decoded = boxxjs.base58decode(data);
			...
			if(bytes && bytes.length>0) {
 				r.version = boxxjs.uint(bytes.slice(0, 4) , 4);
 				r.depth = boxxjs.uint(bytes.slice(4, 5) ,1);
				r.parent_fingerprint = bytes.slice(5, 9);
				r.child_index = boxxjs.uint(bytes.slice(9, 13), 4);
 				r.chain_code = bytes.slice(13, 45);
				r.key_bytes = bytes.slice(45, 78);

				var c = boxxjs.compressed; // get current default
				...
				if(r.key_bytes[0] == 0x00) {
					...
					var pubkey = boxxjs.newPubkey(privkeyHex);

					r.keys = {'privkey':privkeyHex,
						'pubkey':pubkey,
						'address':boxxjs.pubkey2address(pubkey),
						'wif':boxxjs.privkey2wif(privkeyHex)};
					...
					r.keys = {'pubkey': pubkeyHex,
						'address':boxxjs.pubkey2address(pubkeyHex)};
				} else {
					r.type = 'invalid';
				}
				...
				boxxjs.compressed = c; // reset to default
			}
		}
		...
		r.derive = function(i){
			i = (i)?i:0;
			var blob = (Crypto.util.hexToBytes(this.keys.pubkey)).concat(boxxjs.numToBytes(i,4).reverse());
			...
			if(this.type=='private'){
				...
				pubkey = boxxjs.newPubkey(key);

				o.keys = {'privkey':key,
					'pubkey':pubkey,
					'wif':boxxjs.privkey2wif(key),
					'address':boxxjs.pubkey2address(pubkey)};
		...
		r.master = function(pass) {
			var seed = (pass) ? Crypto.SHA256(pass) : boxxjs.newPrivkey();
			...
			return hd.make({'depth':0,
				...
				'pubkey':boxxjs.newPubkey(I.slice(0, 64))});
		}
		...
		r.make = function(data){ // { (int) depth, (array) parent_fingerprint, (int) child_index, (byte array) chain_code, (hex str) privkey, (hex str) pubkey}
			...
			//child index
			k = k.concat((boxxjs.numToBytes(data.child_index, 4)).reverse());
			...
			//encode xprv key
			if(data.privkey){
				var prv = (boxxjs.numToBytes(boxxjs.hdkey.prv, 4)).reverse();
				prv = prv.concat(k);
				prv.push(0x00);
				...
				o.privkey = boxxjs.base58encode(ret);
			}

			//encode xpub key
			if(data.pubkey){
				var pub = (boxxjs.numToBytes(boxxjs.hdkey.pub, 4)).reverse();
				pub = pub.concat(k);
				...
				o.pubkey = boxxjs.base58encode(ret);
			}
			return o;
		}
	...
	boxxjs.script = function(data) {
		...
		} else if (boxxjs.isArray(data)) {
			r.buffer = data;
		} else if (data instanceof boxxjs.script) {
		...
		r.decodeRedeemScript = function(script){
			var r = false;
			try {
				var s = boxxjs.script(Crypto.util.hexToBytes(script));
				if((s.chunks.length>=3) && s.chunks[s.chunks.length-1] == 174){//OP_CHECKMULTISIG
					...
					r.pubkeys = pubkeys;
					var multi = boxxjs.pubkeys2MultisigAddress(pubkeys, r.signaturesRequired);
					...
					r.address = boxxjs.pubkey2address(rs, boxxjs.multisig);
					r.redeemscript = rs;
					...
					r.checklocktimeverify = boxxjs.bytesToNum(s.chunks[0].slice());
					r.address = boxxjs.simpleHodlAddress(r.pubkey, r.checklocktimeverify).address;
					r.type = "hodl__";
				}
			} catch(e) {
				// console.log(e);
		...
		r.spendToScript = function(address){
			var addr = boxxjs.addressDecode(address);
			var s = boxxjs.script();
			if(addr.type == "bech32"){
				s.writeOp(0);
				s.writeBytes(Crypto.util.hexToBytes(addr.redeemscript));
			} else if(addr.version==boxxjs.multisig){ // multisig address
	...
	boxxjs.transaction = function() {
		...
		r.addstealth = function(stealth, value){
			var ephemeralKeyBigInt = BigInteger.fromByteArrayUnsigned(Crypto.util.hexToBytes(boxxjs.newPrivkey()));
			var curve = EllipticCurve.getSECCurveByName("secp256k1");
			...
			var sendaddress = boxxjs.pubkey2address(Crypto.util.bytesToHex(addressPt.getEncoded(true)));
			...
			this.listUnspent(address, function(data){
				...
				for(i=1;i<=unspent.childElementCount;i++){
					...
					if(segwit){
						...
						s.writeOp(0);
						s.writeBytes(boxxjs.numToBytes(u.getElementsByTagName("value")[0].childNodes[0].nodeValue*1, 8));
		...
		r.addUnspentAndSign = function(wif, callback){
			var self = this;
			var address = boxxjs.wif2address(wif);
		...
		r.transactionHash = function(index, sigHashType) {
			...
			var clone = boxxjs.clone(this);
			var shType = sigHashType || 1;

			/* black out all other ins, except this one */
			for (var i = 0; i < clone.ins.length; i++) {
				if(index!=i){
					clone.ins[i].script = boxxjs.script();
				}
			}

			var extract = this.extractScriptKey(index);
			clone.ins[index].script = boxxjs.script(extract['script']);
			...
			if((clone.ins) && clone.ins[index]){
				...
				buffer = buffer.concat(boxxjs.numToBytes(parseInt(shType), 4));
		...
		r.transactionHashSegWitV0 = function(index, sigHashType){
			...
			*/
			...
			var value = boxxjs.numToBytes(extract['value'], 8);

			// start

			var zero = boxxjs.numToBytes(0, 32);
			var version = boxxjs.numToBytes(parseInt(this.version), 4);
			...
			if(!(sigHashType >= 80)){	// not sighash anyonecanpay 
				for(var i = 0; i < this.ins.length; i++){
					...
					bufferTmp = bufferTmp.concat(boxxjs.numToBytes(this.ins[i].outpoint.index, 4));
				}
			}
			...
			if(!(sigHashType >= 80) && sigHashType != 2 && sigHashType != 3){ // not sighash anyonecanpay & single & none
				for(var i = 0; i < this.ins.length; i++){
					bufferTmp = bufferTmp.concat(boxxjs.numToBytes(this.ins[i].sequence, 4));
				}
			}
			...
			outpoint = outpoint.concat(boxxjs.numToBytes(this.ins[index].outpoint.index, 4));

			var nsequence = boxxjs.numToBytes(this.ins[index].sequence, 4);
			var hashOutputs = zero;
			var bufferTmp = [];
			if(sigHashType != 2 && sigHashType != 3){		// not sighash single & none
				for(var i = 0; i < this.outs.length; i++ ){
					bufferTmp = bufferTmp.concat(boxxjs.numToBytes(this.outs[i].value, 8));
					bufferTmp = bufferTmp.concat(boxxjs.numToVarInt(this.outs[i].script.buffer.length));
				...
				bufferTmp = bufferTmp.concat(boxxjs.numToBytes(this.outs[index].value, 8));
				bufferTmp = bufferTmp.concat(boxxjs.numToVarInt(this.outs[i].script.buffer.length));
				bufferTmp = bufferTmp.concat(this.outs[index].script.buffer);
				hashOutputs = Crypto.SHA256(Crypto.SHA256(bufferTmp, {asBytes: true}), {asBytes: true});
			}

			var locktime = boxxjs.numToBytes(this.lock_time, 4);
			var sighash = boxxjs.numToBytes(sigHashType, 4);
		...
		r.extractScriptKey = function(index) {
			if(this.ins[index]){
				if((this.ins[index].script.chunks.length==5) && this.ins[index].script.chunks[4]==172 && boxxjs.isArray(this.ins[index].script.chunks[2])){ //OP_CHECKSIG
					...
					if((this.ins[index].script.chunks[2]) && this.ins[index].script.chunks[2].length==8){
						value = boxxjs.bytesToNum(this.ins[index].script.chunks[2]);  // value found encoded in transaction (THIS IS NON STANDARD)
					}
		...
		r.transactionSig = function(index, wif, sigHashType, txhash){
			...
			if(hash){
				var curve = EllipticCurve.getSECCurveByName("secp256k1");
				var key = boxxjs.wif2privkey(wif);
		...
		r.signmultisig = function(index, wif, sigHashType){
			...
			function scriptListPubkey(redeemScript){
				...
				for(var i=1;i<redeemScript.chunks.length-2;i++){
					r[i] = Crypto.util.hexToBytes(boxxjs.pubkeydecompress(Crypto.util.bytesToHex(redeemScript.chunks[i])));
				}
				return r;
			}
			...
			var pubkeyList = scriptListPubkey(boxxjs.script(redeemScript));
			...
			sigsList[boxxjs.countObject(sigsList)+1] = signature;
			...
			for(x in pubkeyList){
				for(y in sigsList){
					...
					if(boxxjs.verifySignature(sighash, sigsList[y], pubkeyList[x])){
		...
		r.signsegwit = function(index, wif, sigHashType){
			...
			var wif2 = boxxjs.wif2pubkey(wif);
			var segwit = boxxjs.segwitAddress(wif2['pubkey']);
			var bech32 = boxxjs.bech32Address(wif2['pubkey']);
			...
			if((segwit['redeemscript'] == Crypto.util.bytesToHex(this.ins[index].script.chunks[0])) || (bech32['redeemscript'] == Crypto.util.bytesToHex(this.ins[index].script.chunks[0]))){
				...
				if(txhash.result == 1){
					...
					if(!boxxjs.isArray(this.witness)){
						this.witness = [];
					}
					...
					for(var i = 0; i < this.ins.length; i++){
						for(var y = 0; y < this.witness.length; y++){
							if(!witness_used.includes(y)){
								var sw = boxxjs.segwitAddress(this.witness[y][1]);
								var b32 = boxxjs.bech32Address(this.witness[y][1]);
								var rs = '';
								...
								if((sw['redeemscript'] == rs) || (b32['redeemscript'] == rs)){
									...
									if(b32['redeemscript'] == rs){
										this.ins[index].script = boxxjs.script();
									}
									break;
								}
							}
		...
		r.sign = function(wif, sigHashType){
			...
			for (var i = 0; i < this.ins.length; i++) {
				...
				var w2a = boxxjs.wif2address(wif);
				var script = boxxjs.script();
		...
		r.serialize = function(){
			var buffer = [];
			buffer = buffer.concat(boxxjs.numToBytes(parseInt(this.version),4));

			if(boxxjs.isArray(this.witness)){
				buffer = buffer.concat([0x00, 0x01]);
			}

			buffer = buffer.concat(boxxjs.numToVarInt(this.ins.length));
			for (var i = 0; i < this.ins.length; i++) {
				var txin = this.ins[i];
				buffer = buffer.concat(Crypto.util.hexToBytes(txin.outpoint.hash).reverse());
				buffer = buffer.concat(boxxjs.numToBytes(parseInt(txin.outpoint.index),4));
				var scriptBytes = txin.script.buffer;
				buffer = buffer.concat(boxxjs.numToVarInt(scriptBytes.length));
				buffer = buffer.concat(scriptBytes);
				buffer = buffer.concat(boxxjs.numToBytes(parseInt(txin.sequence),4));
			}
			buffer = buffer.concat(boxxjs.numToVarInt(this.outs.length));

			for (var i = 0; i < this.outs.length; i++) {
				var txout = this.outs[i];
 				buffer = buffer.concat(boxxjs.numToBytes(txout.value,8));
				var scriptBytes = txout.script.buffer;
				buffer = buffer.concat(boxxjs.numToVarInt(scriptBytes.length));
				buffer = buffer.concat(scriptBytes);
			}

			if((boxxjs.isArray(this.witness)) && this.witness.length>=1){
				for(var i = 0; i < this.witness.length; i++){
	 				buffer = buffer.concat(boxxjs.numToVarInt(this.witness[i].length));
					for(var x = 0; x < this.witness[i].length; x++){
		 				buffer = buffer.concat(boxxjs.numToVarInt(Crypto.util.hexToBytes(this.witness[i][x]).length));
						buffer = buffer.concat(Crypto.util.hexToBytes(this.witness[i][x]));
					}
				}
			}

			buffer = buffer.concat(boxxjs.numToBytes(parseInt(this.lock_time),4));
		...
		r.deserialize = function(buffer){
			...
			var obj = new boxxjs.transaction();
			...
			for (var i = 0; i < ins; i++) {
				obj.ins.push({
					...
					script: boxxjs.script(readVarString()),
			...
			for (var i = 0; i < outs; i++) {
				obj.outs.push({
					value: boxxjs.bytesToNum(readBytes(8)),
					script: boxxjs.script(readVarString())
				});
			}
			...
			if(witness == true){
				for (i = 0; i < ins; ++i) {
					...
					for(var y = 0; y < count; y++){
						...
						pos += slice;
						if(!boxxjs.isArray(obj.witness[i])){
							obj.witness[i] = [];
						}
	...
	boxxjs.verifySignature = function (hash, sig, pubkey) {
		...
		return boxxjs.verifySignatureRaw(e, r, s, Q);
	}

	boxxjs.verifySignatureRaw = function (e, r, s, Q) {
		var ecparams = EllipticCurve.getSECCurveByName("secp256k1");
		var n = ecparams.getN();
		var G = ecparams.getG();
	...
	boxxjs.base58encode = function(buffer) {
	...
	boxxjs.base58decode = function(buffer){
		var alphabet = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
		var base = BigInteger.valueOf(58);
	...
	/* clone an object */
	boxxjs.clone = function(obj) {
		if(obj == null || typeof(obj) != 'object') return obj;
		var temp = new obj.constructor();

		for(var key in obj) {
			if(obj.hasOwnProperty(key)) {
				temp[key] = boxxjs.clone(obj[key]);
			}
		}
 		return temp;
	}

	boxxjs.numToBytes = function(num,bytes) {
		...
		if (bytes == 0) { 
			...
			return [num % 256].concat(boxxjs.numToBytes(Math.floor(num / 256),bytes-1));
		}
	}

	boxxjs.numToByteArray = function(num) {
		if (num <= 256) { 
			return [num];
		} else {
			return [num % 256].concat(boxxjs.numToByteArray(Math.floor(num / 256)));
		}
	}

	boxxjs.numToVarInt = function(num) {
		if (num < 253) {
			return [num];
		} else if (num < 65536) {
			return [253].concat(boxxjs.numToBytes(num,2));
		} else if (num < 4294967296) {
			return [254].concat(boxxjs.numToBytes(num,4));
		} else {
			return [255].concat(boxxjs.numToBytes(num,8));
		}
	}

	boxxjs.bytesToNum = function(bytes) {
		if (bytes.length == 0) return 0;
		else return bytes[0] + 256 * boxxjs.bytesToNum(bytes.slice(1));
	}

	boxxjs.uint = function(f, size) {
		if (f.length < size)
	...
	boxxjs.isArray = function(o){
		return Object.prototype.toString.call(o) === '[object Array]';
	}

	boxxjs.countObject = function(obj){
		var count = 0;
		var i;
		for (i in obj) {
