$(document).ready(function() {
  ...
  var explorer_addr =
    "https://chainz.cryptoid.info/" + coinID + "/address.dws?";
    ...
    if (email.match(/[\s\w\d]+@[\s\w\d]+/g)) {
      if ($("#openPass").val().length >= 10) {
        if ($("#openPass").val() == $("#openPassConfirm").val()) {
          ...
          var regchars = pass.match(/[a-z]+/g)
            ? pass.match(/[a-z]+/g).length
            : 1;
          var regupchars = pass.match(/[A-Z]+/g)
            ? pass.match(/[A-Z]+/g).length
            : 1;
          var regnums = pass.match(/[0-9]+/g)
          ...
          var keys = boxxjs.newKeys(s);
          var address = keys.address;
          var wif = keys.wif;
          var pubkey = keys.pubkey;
          var privkeyaes = CryptoJS.AES.encrypt(keys.wif, pass);

          $("#walletKeys" + coinID + " .walletSegWitRS").addClass("hidden");

          if ($("#walletSegwit" + coinID).is(":checked")) {
            if ($("#walletSegwitBech32" + coinID).is(":checked")) {
              var sw = boxxjs.bech32Address(pubkey);
              address = sw.address;
            } else {
            ...
            $("#walletKeys" + coinID + " .walletSegWitRS").removeClass(
              "hidden"
            );
            $("#walletKeys" + coinID + " .walletSegWitRS input:text").val(
              sw.redeemscript
            );
          }

          $("#walletAddress" + coinID).html(address);
          $("#walletHistory" + coinID).attr("href", explorer_addr + address);

          $("#walletQrCode" + coinID).html("");
          var qrcode = new QRCode("walletQrCode");
          qrcode.makeCode(coinID + ":" + address);

          $("#walletKeys" + coinID + " .privkey").val(wif);
          $("#walletKeys" + coinID + " .pubkey").val(pubkey);
          $("#walletKeys" + coinID + " .privkeyaes").val(privkeyaes);

          $("#openLogin" + coinID).hide();
          $("#openWallet" + coinID)
            .removeClass("hidden")
            .show();
        ...
        $("#openLoginStatus" + coinID)
          .html("Your password must be at least 10 chars long")
          .removeClass("hidden")
          .fadeOut()
          .fadeIn();
      }
      ...
      $("#openLoginStatus" + coinID)
        .html("Your email address doesn't appear to be valid")
        .removeClass("hidden")
        .fadeOut()
        .fadeIn();
    }

    $("#openLoginStatus" + coinID).prepend(
      '<span class="glyphicon glyphicon-exclamation-sign"></span> '
    );
  });

  $("#walletLogout" + coinID).click(function() {
    $("#openEmail").val("");
    $("#openPass").val("");
    $("#openPassConfirm").val("");
    ...
    $("#walletAddress" + coinID).html("");
    $("#walletHistory" + coinID).attr("href", explorer_addr);

    $("#walletQrCode" + coinID).html("");
    var qrcode = new QRCode("walletQrCode");
    qrcode.makeCode(coinID + ":");

    $("#walletKeys" + coinID + " .privkey").val("");
    $("#walletKeys" + coinID + " .pubkey").val("");
  ...
  function hideAll() {
    ...
    $("#walletSpend" + coinID)
      .removeClass("hidden")
      .addClass("hidden");
    $("#walletTxHistory" + coinID)
      .removeClass("hidden")
      .addClass("hidden");
  }

  $("#walletSegwit" + coinID).click(function() {
    if ($(this).is(":checked")) {
      $(".walletSegwitType" + coinID).attr("disabled", false);
    } else {
      $(".walletSegwitType" + coinID).attr("disabled", true);
    }
  });

  $("#walletToSegWit" + coinID).click(function() {
    $("#walletToBtn" + coinID).html('SegWit <span class="caret"></span>');
    $("#walletSegwit" + coinID)[0].checked = true;
    $("#walletSegwitp2sh" + coinID)[0].checked = true;
    $("#openBtn" + coinID).click();
  });

  $("#walletToSegWitBech32" + coinID).click(function() {
    $("#walletToBtn" + coinID).html('Bech32 <span class="caret"></span>');
    $("#walletSegwit" + coinID)[0].checked = true;
    $("#walletSegwitBech32" + coinID)[0].checked = true;
    $("#openBtn" + coinID).click();
  });

  $("#walletToLegacy" + coinID).click(function() {
    $("#walletToBtn" + coinID).html('Legacy <span class="caret"></span>');
    $("#walletSegwit" + coinID)[0].checked = false;
    $("#openBtn" + coinID).click();
  });

  $("#walletShowKeys" + coinID).click(function() {
    hideAll();
    $("#walletKeys" + coinID).removeClass("hidden");
  });

  $("#walletBalance" + coinID).click(function() {
    walletApiBalance();
  });

  $("#walletTxShow" + coinID).click(function() {
    hideAll();
    if (tx_timer) {
  ...
  $("#walletUncBalance" + coinID).click(function() {
    walletApiUncBalance();
  });
  ...
  $("#newPaperwalletBtnbzx").click(function() {
    ...
    paperwallet.document.write(
      '<h2>BZX PaperWallet</h2><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Address (Share)</h3></div><div style="text-align: center;"><div id="qraddressbzx"></div><p>' +
        $("#newAddressbzx").val() +
        '</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Public Key</h3></div><div style="text-align: center;"><div id="qrpubkeybzx"></div><p>' +
        $("#newPubKeybzx").val() +
        '</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Private Key (KEEP SECRET!)</h3></div><div style="text-align: center;"><div id="qrprivkeybzx"></div><p>' +
        $("#newPrivKeybzx").val() +
        "</p></div></div><hr>" +
    ...
    paperwallet.focus();
    new QRCode(paperwallet.document.getElementById("qraddressbzx"), {
      text: $("#newAddressbzx").val(),
      width: 200,
      height: 200
    });
    new QRCode(paperwallet.document.getElementById("qrpubkeybzx"), {
      text: $("#newPubKeybzx").val(),
      width: 200,
      height: 200
    });
    new QRCode(paperwallet.document.getElementById("qrprivkeybzx"), {
      text: $("#newPrivKeybzx").val(),
      width: 200,
      height: 200
    });
    paperwallet.print();
  ...
  $("#newPaperwalletBtngxx").click(function() {
    ...
    var paperwallet = window.open();
    paperwallet.document.write(
      '<h2>GXX PaperWallet</h2><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Address (Share)</h3></div><div style="text-align: center;"><div id="qraddressgxx"></div><p>' +
        $("#newAddressgxx").val() +
        '</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Public Key</h3></div><div style="text-align: center;"><div id="qrpubkeygxx"></div><p>' +
        $("#newPubKeygxx").val() +
        '</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Private Key (KEEP SECRET!)</h3></div><div style="text-align: center;"><div id="qrprivkeygxx"></div><p>' +
        $("#newPrivKeygxx").val() +
        "</p></div></div><hr>" +
    ...
    paperwallet.document.close();
    paperwallet.focus();
    new QRCode(paperwallet.document.getElementById("qraddressgxx"), {
      text: $("#newAddressgxx").val(),
      width: 200,
      height: 200
    });
    new QRCode(paperwallet.document.getElementById("qrpubkeygxx"), {
      text: $("#newPubKeygxx").val(),
      width: 200,
      height: 200
    });
    new QRCode(paperwallet.document.getElementById("qrprivkeygxx"), {
      text: $("#newPrivKeygxx").val(),
      width: 200,
      height: 200
    });
    paperwallet.print();
  ...
  $("#newPaperwalletBtnbcz").click(function() {
    ...
    var paperwallet = window.open();
    paperwallet.document.write(
      '<h2>BCZ PaperWallet</h2><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Address (Share)</h3></div><div style="text-align: center;"><div id="qraddressbcz"></div><p>' +
        $("#newAddressbcz").val() +
        '</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Public Key</h3></div><div style="text-align: center;"><div id="qrpubkeybcz"></div><p>' +
        $("#newPubKeybcz").val() +
        '</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Private Key (KEEP SECRET!)</h3></div><div style="text-align: center;"><div id="qrprivkeybcz"></div><p>' +
        $("#newPrivKeybcz").val() +
        "</p></div></div><hr>" +
    ...
    paperwallet.document.close();
    paperwallet.focus();
    new QRCode(paperwallet.document.getElementById("qraddressbcz"), {
      text: $("#newAddressbcz").val(),
      width: 200,
      height: 200
    });
    new QRCode(paperwallet.document.getElementById("qrpubkeybcz"), {
      text: $("#newPubKeybcz").val(),
      width: 200,
      height: 200
    });
    new QRCode(paperwallet.document.getElementById("qrprivkeybcz"), {
      text: $("#newPrivKeybcz").val(),
      width: 200,
      height: 200
    });
    paperwallet.print();
        ...
        if (dvalue >= total) {
          ...
          tx2.broadcast(function(data) {
            if (
              ...
              $("#walletSendConfirmStatusbzx")
                .removeClass("hidden")
                .addClass("alert-success")
                .html(
                  'txid: <a href="https://chainz.cryptoid.info/bzx/tx.dws?' +
                    $(data)
                      .find("txid")
                      .text() +
                    '" target="_blank" style="color:#8ed7f1 !important;font-size:70%;">' +
                    $(data)
                      .find("txid")
                      .text() +
                    "</a>"
        ...
        if (dvalue >= total) {
          ...
          tx2.broadcast(function(data) {
            if (
              ...
              $("#walletSendConfirmStatusgxx")
                .removeClass("hidden")
                .addClass("alert-success")
                .html(
                  'txid: <a href="https://chainz.cryptoid.info/gxx/tx.dws?' +
                    $(data)
                      .find("txid")
                      .text() +
                    '" target="_blank" style="color:#ffc107 !important;font-size:70%;">' +
                    $(data)
                      .find("txid")
                      .text() +
                    "</a>"
        ...
        if (dvalue >= total) {
          ...
          tx2.broadcast(function(data) {
            if (
              ...
              $("#walletSendConfirmStatusbcz")
                .removeClass("hidden")
                .addClass("alert-success")
                .html(
                  'txid: <a href="https://chainz.cryptoid.info/bcz/tx.dws?' +
                    $(data)
                      .find("txid")
                      .text() +
                    '" target="_blank" style="color:#3cd5b0 !important;font-size:70%;">' +
                    $(data)
                      .find("txid")
                      .text() +
                    "</a>"
        ...
        $("#walletSendStatusbzx")
          ...
          .html(
            "You are trying to spend " +
              total +
              " but have a balance of " +
              balance
          );
      }
    } else {
      $("#walletSpendbzx" + " .has-error")
        .fadeOut()
        .fadeIn();
      $("#walletSendStatusbzx")
        .removeClass("hidden")
        .html(
          '<span class="glyphicon glyphicon-exclamation-sign"></span> One or more input has an error'
        );
    }
  });
  ...
  $("#walletSpendTo" + coinID + " .addressAdd").click(function() {
    ...
    $("#walletSpendTo" + coinID + " .glyphicon-plus:last")
      .removeClass("glyphicon-plus")
      .addClass("glyphicon-minus");
    $("#walletSpendTo" + coinID + " .glyphicon-minus:last")
      .parent()
      .removeClass("addressAdd")
      .addClass("addressRemove");
  ...
  function walletApiBalance() {
    ...
    $.ajax({
      type: "GET",
      url:
        "https://apixx.chaintoolz.com/api/?s=" +
        coinID +
        "&q=getbalance&a=" +
        $("#walletAddress" + coinID).html(),
      dataType: "json",
      error: function(data) {
        $("#redeemFromStatus" + coinID)
          .removeClass("hidden")
          .html(
            '<span class="glyphicon glyphicon-exclamation-sign"></span> Unexpected error, unable to retrieve unspent outputs!'
          );
      },
      success: function(data) {
        $("#walletBalance" + coinID)
          .html(data.balance.confirmed + " BZX")
          .fadeOut()
          .fadeIn();
        //$("#walletUncBalance"+coinID).html(data.balance.unconfirmed+" BZX").fadeOut().fadeIn();

        $("#walletLoader" + coinID).addClass("hidden");
      },
      complete: function(data, status) {
        $("#redeemFromBtn" + coinID)
          .html("Load")
          .attr("disabled", false);
        totalInputAmount();
      }
    ...
    $.ajax({
      ...
      success: function(data) {
        $("#walletBalance" + coinID)
          ...
          .fadeOut()
          .fadeIn();
        $("#walletUncBalance" + coinID)
          .html(data.balance.unconfirmed + " BZX")
          .fadeOut()
          .fadeIn();
  ...
  function addTxHistory(tx, conf) {
    ...
    c2 = row.insertCell();
    c1.innerHTML =
      '<a href="' + explorer_tx + tx + '" />' + tx.slice(0, 14) + "</a>";
    c2.innerHTML = conf;
    if (conf < 6) {
      c2.style.color = "red";
    } else {
  ...
  $("#newKeysBtnbzx").click(function() {
    ...
    var s = $("#newBrainwallet").is(":checked")
      ? $("#brainwallet").val()
      : null;
    ...
    /* encrypted key code */
    if (
      !$("#encryptKey").is(":checked") ||
      $("#aes256passbzx").val() == $("#aes256pass_confirmbzx").val()
    ) {
    ...
    $("#newPrivKeyEncbzx").val(
      CryptoJS.AES.encrypt(coin.wif, $("#aes256passbzx").val()) + ""
    );
  });
  ...
  $("#newKeysBtngxx").click(function() {
    ...
    if (
      !$("#encryptKey").is(":checked") ||
      $("#aes256passgxx").val() == $("#aes256pass_confirmgxx").val()
    ) {
      $("#aes256passStatus").addClass("hidden");
      if ($("#encryptKey").is(":checked")) {
        $("#aes256wifkey").removeClass("hidden");
    ...
    $("#newPrivKeyEncgxx").val(
      CryptoJS.AES.encrypt(coin.wif, $("#aes256passgxx").val()) + ""
    );
  });
  ...
  $("#newKeysBtnbcz").click(function() {
    ...
    if (
      !$("#encryptKey").is(":checked") ||
      $("#aes256passbcz").val() == $("#aes256pass_confirmbcz").val()
    ) {
      $("#aes256passStatus").addClass("hidden");
      if ($("#encryptKey").is(":checked")) {
        $("#aes256wifkey").removeClass("hidden");
    ...
    $("#newPrivKeyEncbcz").val(
      CryptoJS.AES.encrypt(coin.wif, $("#aes256passbcz").val()) + ""
    );
  });
  ...
  $("#newSegWitKeysBtn").click(function() {
    ...
    var s = $("#newSegWitBrainwallet").is(":checked")
      ? $("#brainwalletSegWit").val()
      : null;
    var coin = boxxjs.newKeys(s);
  ...
  $("#newMultiSigAddress").click(function() {
    ...
    if (
      isNaN($("#releaseCoins option:selected").html()) ||
      (!isNaN($("#releaseCoins option:selected").html()) &&
        ($("#releaseCoins option:selected").html() >
          $("#multisigPubKeys .pubkey").length ||
          $("#releaseCoins option:selected").html() * 1 <= 0 ||
          $("#releaseCoins option:selected").html() * 1 > 8))
    ) {
      $("#releaseCoins")
        .parent()
        .addClass("has-error");
      $("#multiSigErrorMsg")
        .html(
          '<span class="glyphicon glyphicon-exclamation-sign"></span> Minimum signatures required is greater than the amount of public keys provided'
        )
        .fadeIn();
      return false;
    }
    ...
    if (
      ...
      if (multisig.size <= 520) {
        ...
        $("#multiSigErrorMsg")
          .html(
            '<span class="glyphicon glyphicon-exclamation-sign"></span> Your generated redeemscript is too large (>520 bytes) it can not be used safely'
          )
          .fadeIn();
      }
    } else {
      $("#multiSigErrorMsg")
        .html(
          '<span class="glyphicon glyphicon-exclamation-sign"></span> One or more public key is invalid!'
        )
        .fadeIn();
    }
  });
  ...
  $("#multisigPubKeys .pubkeyAdd").click(function() {
    if ($("#multisigPubKeys .pubkeyRemove").length < 14) {
      ...
      $("#multisigPubKeys .glyphicon-plus:last")
        .removeClass("glyphicon-plus")
        .addClass("glyphicon-minus");
      $("#multisigPubKeys .glyphicon-minus:last")
        .parent()
        .removeClass("pubkeyAdd")
        .addClass("pubkeyRemove");
  ...
  $("#newTimeLockedAddress").click(function() {
    ...
    if ($("#timeLockedRbTypeDate").is(":checked")) {
      // by date
      var date = $("#timeLockedDateTimePicker")
        .data("DateTimePicker")
        .date();
    ...
    if (
      ...
      try {
        ...
        $("#timeLockedErrorMsg")
          .html(
            '<span class="glyphicon glyphicon-exclamation-sign"></span> ' + e
          )
          .fadeIn();
      }
    } else {
      $("#timeLockedErrorMsg")
        .html(
          '<span class="glyphicon glyphicon-exclamation-sign"></span> Public key and/or date is invalid!'
        )
        .fadeIn();
    }
  });
  ...
  $("#newHDKeysBtn").click(function() {
    boxxjs.compressed = true;
    var s = $("#newHDBrainwallet").is(":checked")
      ? $("#HDBrainwallet").val()
      : null;
    var hd = boxxjs.hd();
  ...
  $("#recipients .addressAddTo").click(function() {
    if ($("#recipients .addressRemoveTo").length < 19) {
      ...
      $("#recipients .glyphicon-plus:last")
        .removeClass("glyphicon-plus")
        .addClass("glyphicon-minus");
      $("#recipients .glyphicon-minus:last")
        .parent()
        .removeClass("addressAdd")
        .addClass("addressRemoveTo");
  ...
  $("#inputs .txidAdd").click(function() {
    ...
    $("#inputs .glyphicon-plus:last")
      .removeClass("glyphicon-plus")
      .addClass("glyphicon-minus");
    $("#inputs .glyphicon-minus:last")
      .parent()
      .removeClass("txidAdd")
      .addClass("txidRemove");
  ...
  $("#transactionBtn").click(function() {
    ...
    $('#putTabs a[href="#txinputs"], #putTabs a[href="#txoutputs"]').attr(
      "style",
      ""
    );
    ...
    $.each($("#inputs .row"), function(i, o) {
      ...
      if (!$(o).hasClass("has-error")) {
        ...
        if (currentScript.match(/^76a914[0-9a-f]{40}88ac$/)) {
          ...
          var scriptSigSize =
            parseInt(currentScript.slice(1, 2), 16) * 74 +
            parseInt(currentScript.slice(-3, -2), 16) * 34 +
            43;
    ...
    $.each($("#recipients .row"), function(i, o) {
      ...
      if (
        a != "" &&
        (ad.version == boxxjs.pub ||
          ad.version == boxxjs.multisig ||
          ad.type == "bech32") &&
        $(".amount", o).val() != ""
      ) {
        // address
    ...
    if (!$("#recipients .row, #inputs .row").hasClass("has-error")) {
      ...
      $("#transactionCreateStatus")
        .removeClass("hidden")
        .html("One or more input or output is invalid")
        .fadeOut()
        .fadeIn();
    }
  });
  ...
  $("#donateTxBtn").click(function() {
    ...
    if (!exists) {
      ...
      $("#recipients .recipient:last .address:last")
        .val(boxxjs.developer)
        .fadeOut()
        .fadeIn();
      ...
      MediaStreamTrack.getSources(function(sourceInfos) {
        ...
        for (var i = 0; i !== sourceInfos.length; ++i) {
          ...
          if (sourceInfo.kind === "video") {
            option.text =
              sourceInfo.label ||
              "camera " + ($("select#videoSource options").length + 1);
  ...
  function scannerStart() {
    ...
    if (navigator.getUserMedia) {
      ...
      QCodeDecoder().decodeFromCamera(
        document.getElementById("videoReader"),
        function(er, data) {
          if (!er) {
  ...
  $("#redeemFromBtnbzx").click(function() {
    ...
    if (redeem.from == "other") {
      $("#redeemFromStatusbzx")
        .removeClass("hidden")
        .html(
          '<span class="glyphicon glyphicon-exclamation-sign"></span> The private key you have entered is invalid'
        );
      return false;
    }
  ...
  function redeemingFrom(string) {
    ...
    if (decode.version == boxxjs.pub) {
      // regular address
      r.addr = string;
      r.from = "address";
      r.redeemscript = false;
    } else if (decode.version == boxxjs.priv) {
      // wif key
      ...
      r.from = "wif";
      r.redeemscript = false;
    } else if (decode.version == boxxjs.multisig) {
      // mulisig address
      r.addr = "";
  ...
  function mediatorPayment(redeem) {
    if (redeem.from == "redeemScript") {
      $('#recipients .row[rel="' + redeem.addr + '"]')
        .parent()
        .remove();
      ...
      $.each(redeem.decodedRs.pubkeys, function(i, o) {
        $.each($("#mediatorList option"), function(mi, mo) {
          ...
          var payto = boxxjs.pubkey2address(pubkey); // pay to mediators address

          if (o == pubkey) {
            // matched a mediators pubkey?

            var clone =
              '<span><div class="row recipients mediator mediator_' +
              pubkey +
              '" rel="' +
              redeem.addr +
              '">' +
              $("#recipients .addressAddTo")
                .parent()
                .parent()
                .html() +
              "</div><br></span>";
            ...
            $(
              "#recipients .mediator_" + pubkey + " .glyphicon-plus:first"
            ).removeClass("glyphicon-plus");
            $("#recipients .mediator_" + pubkey + " .address:first")
              .val(payto)
              .attr("disabled", true)
              .attr("readonly", true)
              .attr("title", "Medation fee for " + $(mo).html());

            var amount = ((fee * $("#totalInput").html()) / 100).toFixed(8);
            $("#recipients .mediator_" + pubkey + " .amount:first")
              .attr("disabled", amount * 1 == 0 ? false : true)
              .val(amount)
              .attr("title", "Medation fee for " + $(mo).html());
          }
        });
  ...
  function addOutput(tx, n, script, amount) {
    if (tx) {
      ...
      if (
        (script.match(/^00/) && script.length == 44) ||
        (script.length == 40 && script.match(/^[a-f0-9]+$/gi))
      ) {
        s = boxxjs.script();
        ...
        if (data.status && data.data && data.status == "success") {
          ...
          for (var i in data.data.txs) {
            ...
            if (tx.match(/^[a-f0-9]+$/)) {
              ...
              var n = o.output_no;
              var script =
                redeem.redeemscript == true ? redeem.decodedRs : o.script_hex;
              var amount = o.value;
  ...
  /*	function rawSubmitDefault(thisbtn){
		...
		//var rawtxhex = { tx_hex: $("#rawTransaction").val()}
                $.ajax ({
                        type: "POST",
                        url: "https://"+coinID+".apixx.ovh/api/send/",
...
*/
  ...
  function decodeRedeemScript() {
    ...
    if (decode) {
      ...
      if (decode.type == "multisig__") {
        ...
        $("#verifyRsDataMultisig .signaturesRequired").html(
          decode["signaturesRequired"]
        );
        ...
        for (var i = 0; i < decode.pubkeys.length; i++) {
          ...
          $(
            '<tr><td width="30%"><input type="text" class="form-control" value="' +
              address +
              '" readonly></td><td><input type="text" class="form-control" value="' +
              pubkey +
              '" readonly></td></tr>'
          ).appendTo("#verifyRsDataMultisig table tbody");
        }
        ...
        $("#verifyRsDataHodl .pubkey").val(
          boxxjs.pubkey2address(decode["pubkey"])
        );
        $("#verifyRsDataHodl .date").val(
          decode["checklocktimeverify"] >= 500000000
            ? moment
                .unix(decode["checklocktimeverify"])
                .format("MM/DD/YYYY HH:mm")
            : decode["checklocktimeverify"]
        );
        $("#verifyRsData").removeClass("hidden");
  ...
  function decodeTransactionScript() {
    ...
    try {
      ...
      $("#verifyTransactionData .transactionSize").html(
        decode.size() + " <i>bytes</i>"
      );
      $("#verifyTransactionData .transactionLockTime").html(
        decode["lock_time"]
      );
      ...
      $.each(decode.ins, function(i, o) {
        ...
        h +=
          ...
          Crypto.util.bytesToHex(o.script.buffer) +
          '" readonly></td>';
        h +=
          '<td class="col-xs-1"> <span class="glyphicon glyphicon-' +
          (s.signed == "true" ||
          (decode.witness[i] && decode.witness[i].length == 2)
            ? "ok"
            : "remove") +
          '-circle"></span>';
      ...
      $.each(decode.outs, function(i, o) {
        if (o.script.chunks.length == 2 && o.script.chunks[0] == 106) {
          // OP_RETURN

          var data = Crypto.util.bytesToHex(o.script.chunks[1]);
          var dataascii = hex2ascii(data);

          if (dataascii.match(/^[\s\d\w]+$/gi)) {
            data = dataascii;
          }

          h += "<tr>";
          h +=
            '<td><input type="text" class="form-control" value="(OP_RETURN) ' +
            data +
            '" readonly></td>';
          h +=
            '<td class="col-xs-1">' +
            (o.value / 100000000).toFixed(8) +
            "</td>";
          h +=
            '<td class="col-xs-2"><input class="form-control" type="text" value="' +
            Crypto.util.bytesToHex(o.script.buffer) +
          ...
          if (o.script.chunks.length == 5) {
            addr = boxxjs.scripthash2address(
              Crypto.util.bytesToHex(o.script.chunks[2])
            );
          } else if (o.script.chunks.length == 2 && o.script.chunks[0] == 0) {
            addr = boxxjs.bech32_encode(
              boxxjs.bech32.hrp,
              [boxxjs.bech32.version].concat(
                boxxjs.bech32_convert(o.script.chunks[1], 8, 5, true)
              )
            );
          } else {
            var pub = boxxjs.pub;
            ...
            addr = boxxjs.scripthash2address(
              Crypto.util.bytesToHex(o.script.chunks[1])
            );
            boxxjs.pub = pub;
          }
  ...
  function decodePrivKey() {
    ...
    if (wif.length == 51 || wif.length == 52) {
      try {
        ...
        $("#verifyPrivKey .iscompressed").html(
          w2address["compressed"] ? "true" : "false"
        );
  ...
  function decodeHDaddress() {
    ...
    try {
      var hex = Crypto.util.bytesToHex(boxxjs.base58decode(s).slice(0, 4));
      var hex_cmp_prv = Crypto.util.bytesToHex(
        boxxjs.numToBytes(boxxjs.hdkey.prv, 4).reverse()
      );
      var hex_cmp_pub = Crypto.util.bytesToHex(
        boxxjs.numToBytes(boxxjs.hdkey.pub, 4).reverse()
      );
      if (hex == hex_cmp_prv || hex == hex_cmp_pub) {
        var hd = boxxjs.hd(s);
        $("#verifyHDaddress .hdKey").html(s);
        $("#verifyHDaddress .chain_code").val(
          Crypto.util.bytesToHex(hd.chain_code)
        );
        ...
        $("#verifyHDaddress .key_type").html(
          (
            (hd.depth == 0 && hd.child_index == 0 ? "Master" : "Derived") +
            " " +
            hd.type
          ).toLowerCase()
        );
        $("#verifyHDaddress .parent_fingerprint").val(
          Crypto.util.bytesToHex(hd.parent_fingerprint)
        );
  ...
  function deriveHDaddress() {
    var hd = boxxjs.hd($("#verifyHDaddress .hdKey").html());
    ...
    for (var i = index_start; i <= index_end; i++) {
      ...
      html +=
        ...
        derived.keys.address +
        '" readonly></td>';
      html +=
        '<td><input type="text" class="form-control" value="' +
        (derived.keys.wif ? derived.keys.wif : "") +
        '" readonly></td>';
      html +=
        '<td><input type="text" class="form-control" value="' +
        derived.keys_extended.pubkey +
        '" readonly></td>';
      html +=
        '<td><input type="text" class="form-control" value="' +
        (derived.keys_extended.privkey ? derived.keys_extended.privkey : "") +
        '" readonly></td>';
      html += "</tr>";
    }
  ...
  $("#signBtn").click(function() {
    ...
    if ($("#sign .has-error").length == 0) {
      ...
      try {
        ...
        var signed = t.sign(
          wifkey.val(),
          $("#sighashType option:selected").val()
        );
  ...
  $("#sighashType").change(function() {
    $("#sighashTypeInfo")
      .html($("option:selected", this).attr("rel"))
      .fadeOut()
      .fadeIn();
  });
  ...
  function _get(value) {
    var dataArray = document.location.search.match(
  ...
  $("#settingsBtn").click(function() {
    ...
    if ($("#settings .has-error").length == 0) {
      ...
      $("#statusSettings")
        ...
        .removeClass("hidden")
        .html(
          '<span class="glyphicon glyphicon-ok"></span> Settings updates successfully'
        )
        .fadeOut()
        .fadeIn();
    } else {
      $("#statusSettings")
        .addClass("alert-danger")
        .removeClass("hidden")
        .html("There is an error with one or more of your settings");
    }
  });
  ...
  $("#boxxjs_coin").change(function() {
    ...
    if (o[5] == "false") {
      $("#boxxjs_broadcast, #rawTransaction, #rawSubmitBtn, #openBtn").attr(
        "disabled",
        true
      );
    ...
    if (o[6] == "false") {
      $(
        "#boxxjs_utxo, #redeemFrom" +
          coinID +
          ", #redeemFromBtn" +
          coinID +
          ", #openBtn, .qrcodeScanner" +
          coinID
      ).attr("disabled", true);
