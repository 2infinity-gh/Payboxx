...
setCurrentTab(coinID);
			...
			if($("#openPass").val().length>=10){
				if($("#openPass").val()==$("#openPassConfirm").val()){
					...
					if($("#walletSegwit"+coinID).is(":checked")){
						...
						$("#walletKeys"+coinID+" .walletSegWitRS").removeClass("hidden");
						$("#walletKeys"+coinID+" .walletSegWitRS input:text").val(sw.redeemscript);
					}
					...
					$("#openWallet"+coinID).removeClass("hidden").show();

					walletApiBalance();
					checkBalanceLoop();
				} else {
					$("#openLoginStatus"+coinID).html("Your passwords do not match!").removeClass("hidden").fadeOut().fadeIn();
				}
			} else {
				$("#openLoginStatus"+coinID).html("Your password must be at least 10 chars long").removeClass("hidden").fadeOut().fadeIn();
			}
		} else {
			$("#openLoginStatus"+coinID).html("Your email address doesn't appear to be valid").removeClass("hidden").fadeOut().fadeIn();
		}

		$("#openLoginStatus"+coinID).prepend('<span class="glyphicon glyphicon-exclamation-sign"></span> ');
	});
	...
	$("#walletLogout"+coinID).click(function(){
		...
		$("#openLoginStatus"+coinID).html("").hide();
	});

	function hideAll(){
		$("#walletKeys"+coinID).removeClass("hidden").addClass("hidden");
		$("#walletSpend"+coinID).removeClass("hidden").addClass("hidden");
		$("#walletTxHistory"+coinID).removeClass("hidden").addClass("hidden");
	}
...
///// CONFIRMSEND BZX
	...
	function walletApiBalance() {
	 ...
	 $.ajax ({
		 ...
		 success: function(data) {
			$("#walletBalance"+coinID).html(data.balance.confirmed+" BZX").fadeOut().fadeIn();
	...
	$("#newKeysBtnbzx").click(function(){
		...
		$("#newPrivKeyEncbxz").val(CryptoJS.AES.encrypt(coin.wif, $("#aes256passbxz").val())+'');
...
*/
	...
	function decodeHDaddress(){
		...
		try {
			...
			var hex_cmp_prv = Crypto.util.bytesToHex((boxxjs.numToBytes(boxxjs.hdkey.prv,4)).reverse());
			var hex_cmp_pub = Crypto.util.bytesToHex((boxxjs.numToBytes(boxxjs.hdkey.pub,4)).reverse());
